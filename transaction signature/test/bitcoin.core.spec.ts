...
describe('firovm-core', () => {
  ...
  describe('address.toBase58Check', () => {
    ...
    base58KeysValid.forEach(f => {
      ...
      const network: any = params.isTestnet
        ? firovm.networks.testnet
        : firovm.networks.firovm;
      const version = network[typeMap[params.addrType]];

      it('can export ' + expected, () => {
        assert.strictEqual(
          firovm.address.toBase58Check(hash, version),
          expected,
        );
      });
    });
  ...
  describe('address.fromBase58Check', () => {
    const allowedNetworks = [
      firovm.networks.firovm.pubKeyHash,
      firovm.networks.firovm.scriptHash,
      firovm.networks.testnet.pubKeyHash,
      firovm.networks.testnet.scriptHash,
    ];
    ...
    base58KeysInvalid.forEach(f => {
      ...
      it('throws on ' + strng, () => {
        assert.throws(() => {
          const address = firovm.address.fromBase58Check(strng);
  ...
  describe('Block.fromHex', () => {
    blocksValid.forEach(f => {
      it('can parse ' + f.id, () => {
        const block = firovm.Block.fromHex(f.hex);
  ...
  describe('Transaction.fromHex', () => {
    txValid.forEach(f => {
      ...
      it('can decode ' + fhex, () => {
        const transaction = firovm.Transaction.fromHex(fhex as string);
  ...
  describe('Transaction', () => {
    sigHash.forEach(f => {
      ...
      const hashTypes = [];
      if ((hashType & 0x1f) === firovm.Transaction.SIGHASH_NONE)
        hashTypes.push('SIGHASH_NONE');
      else if ((hashType & 0x1f) === firovm.Transaction.SIGHASH_SINGLE)
        hashTypes.push('SIGHASH_SINGLE');
      else hashTypes.push('SIGHASH_ALL');
      if (hashType & firovm.Transaction.SIGHASH_ANYONECANPAY)
      ...
      it(
        ...
        () => {
          const transaction = firovm.Transaction.fromHex(txHex);
          assert.strictEqual(transaction.toHex(), txHex);

          const script = Buffer.from(scriptHex, 'hex');
          const scriptChunks = firovm.script.decompile(script);
          assert.strictEqual(
            firovm.script.compile(scriptChunks!).toString('hex'),
            scriptHex,
          );
  ...
  describe('script.signature.decode', () => {
    sigCanonical.forEach(hex => {
      ...
      it('can parse ' + hex, () => {
        const parsed = firovm.script.signature.decode(buffer);
        const actual = firovm.script.signature.encode(
          parsed.signature,
          parsed.hashType,
        );
    ...
    sigNoncanonical.forEach((hex, i) => {
      ...
      it('throws on ' + description, () => {
        ...
        assert.throws(() => {
          firovm.script.signature.decode(buffer);
        }, reg);
      });
    });
  });
