...
describe('firovmjs-lib (transactions with psbt)', () => {
  it('can create a 1-to-1 Transaction', () => {
    const alice = ECPair.fromWIF(
      'L2uPYXe17xSTqbCjZvL2DsyXPCbXspvcu5mHLDYUgzdUbZGSKrSr',
    );
    const psbt = new firovm.Psbt();
  ...
  it('can create (and broadcast via 3PBP) a typical Transaction', async () => {
    ...
    const psbt = new firovm.Psbt({ network: regtest })
    ...
    // each signer imports
    const signer1 = firovm.Psbt.fromBase64(psbtBaseText);
    const signer2 = firovm.Psbt.fromBase64(psbtBaseText);
    ...
    const final1 = firovm.Psbt.fromBase64(s1text);
    const final2 = firovm.Psbt.fromBase64(s2text);
    ...
    // to build and broadcast to the actual firovm network, see https://github.com/firovmjs/firovmjs-lib/issues/839
  });
  ...
  it('can create (and broadcast via 3PBP) a Transaction with an OP_RETURN output', async () => {
    ...
    const data = Buffer.from('firovmjs-lib', 'utf8');
    const embed = firovm.payments.embed({ data: [data] });
  ...
  it('can create (and broadcast via 3PBP) a Transaction, w/ a P2SH(P2MS(2 of 4)) (multisig) input', async () => {
    ...
    // build and broadcast to the firovm RegTest network
...
function createPayment(_type: string, myKeys?: any[], network?: any): any {
  ...
  splitType.forEach(type => {
    if (type.slice(0, 4) === 'p2ms') {
      payment = firovm.payments.p2ms({
        m,
        pubkeys: keys.map(key => key.publicKey).sort((a, b) => a.compare(b)),
        network,
      });
    } else if (['p2sh', 'p2wsh'].indexOf(type) > -1) {
      payment = (firovm.payments as any)[type]({
        redeem: payment,
        network,
      });
    } else {
