...
- local regtest-server docker container running
  - `docker run -d -p 8080:8080 junderw/firovmjs-regtest-server`
- node >= v14
...
// firovmjs-lib v6
const firovm = require('firovmjs-lib');
...
const address = firovm.address.fromOutputScript(
  output,
...
function createKeySpendOutput(publicKey) {
  ...
  const commitHash = firovm.crypto.taggedHash('TapTweak', myXOnlyPubkey);
...
function signTweaked(messageHash, key) {
  ...
  const tweakHash = firovm.crypto.taggedHash(
    'TapTweak',
...
function createSigned(key, txid, vout, amountToSend, scriptPubkeys, values) {
  ...
  const sighash = tx.hashForWitnessV1(
    ...
    firovm.Transaction.SIGHASH_DEFAULT // sighash flag, DEFAULT is schnorr-only (DEFAULT == ALL)
  );
