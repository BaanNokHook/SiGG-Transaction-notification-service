...
function getAddress(node: any, network?: any): string {
  return firovm.payments.p2pkh({ pubkey: node.publicKey, network }).address!;
}

describe('firovmjs-lib (BIP32)', () => {
  it('can import a BIP32 testnet xpriv and export to WIF', () => {
    ...
    const node = bip32.fromBase58(xpriv, firovm.networks.testnet);
  ...
  it('can create a BIP32, firovm, account 0, external address', () => {
    const path = "m/0'/0/0";
  ...
  it('can create a BIP44, firovm, account 0, external address', () => {
    const root = bip32.fromSeed(
      Buffer.from(
        'dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd',
        'hex',
  ...
  it('can create a BIP49, firovm testnet, account 0, external address', () => {
    const mnemonic =
    ...
    const { address } = firovm.payments.p2sh({
      redeem: firovm.payments.p2wpkh({
        pubkey: child.publicKey,
        network: firovm.networks.testnet,
      }),
      network: firovm.networks.testnet,
    });
