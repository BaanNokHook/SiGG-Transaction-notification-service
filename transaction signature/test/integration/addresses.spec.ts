...
const TESTNET = firovm.networks.testnet;

describe('firovmjs-lib (addresses)', () => {
  it(
    'can generate a random address [and support the retrieval of ' +
      'transactions for that address (via 3PBP)]',
    async () => {
      const keyPair = ECPair.makeRandom();
      const { address } = firovm.payments.p2pkh({ pubkey: keyPair.publicKey });

      // firovm P2PKH addresses start with a '1'
  ...
  it('can generate a P2SH, pay-to-multisig (2-of-3) address', () => {
    ...
    const { address } = firovm.payments.p2sh({
      redeem: firovm.payments.p2ms({ m: 2, pubkeys }),
    });
  ...
  it('can generate a SegWit address', () => {
    ...
    const { address } = firovm.payments.p2wpkh({ pubkey: keyPair.publicKey });
  ...
  it('can generate a SegWit address (via P2SH)', () => {
    ...
    const { address } = firovm.payments.p2sh({
      redeem: firovm.payments.p2wpkh({ pubkey: keyPair.publicKey }),
    });
  ...
  it('can generate a P2WSH (SegWit), pay-to-multisig (3-of-4) address', () => {
    ...
    const { address } = firovm.payments.p2wsh({
      redeem: firovm.payments.p2ms({ m: 3, pubkeys }),
    });
  ...
  it('can generate a Testnet address', () => {
    ...
    const { address } = firovm.payments.p2pkh({
      pubkey: keyPair.publicKey,
      network: TESTNET,
    });

    // firovm testnet P2PKH addresses start with a 'm' or 'n'
    assert.strictEqual(
  ...
  it('can generate a Litecoin address', () => {
    // WARNING: although possible, firovmjs is NOT necessarily compatible with Litecoin
    const LITECOIN = {
