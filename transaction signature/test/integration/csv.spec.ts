...
import * as firovm from '../..';
import { regtestUtils } from './_regtest';

const ECPair = ECPairFactory(ecc);
const regtest = regtestUtils.network;
const bip68 = require('bip68');
const varuint = require('varuint-firovm');

function toOutputScript(address: string): Buffer {
  return firovm.address.toOutputScript(address, regtest);
}
...
describe('firovmjs-lib (transactions w/ CSV)', () => {
  // force update MTP
  before(async () => {
    await regtestUtils.mine(11);
  });

  const hashType = firovm.Transaction.SIGHASH_ALL;
  ...
  function csvCheckSigOutput(
    ...
    return firovm.script.fromASM(
      `
      OP_IF
          ${firovm.script.number.encode(sequence).toString('hex')}
  ...
  // Ref: https://github.com/firovmbook/firovmbook/blob/f8b883dcd4e3d1b9adf40fed59b7e898fbd9241f/ch07.asciidoc#complex-script-example

  // Note: firovmjs-lib will not offer specific support for problems with
    ...
    return firovm.script.fromASM(
      ...
      OP_IF
          OP_IF
              OP_2
          OP_ELSE
              ${firovm.script.number.encode(sequence1).toString('hex')}
              OP_CHECKSEQUENCEVERIFY
              OP_DROP
              ${_alice.publicKey.toString('hex')}
              OP_CHECKSIGVERIFY
          ...
          OP_CHECKMULTISIG
      OP_ELSE
          ${firovm.script.number.encode(sequence2).toString('hex')}
          OP_CHECKSEQUENCEVERIFY
          OP_DROP
          ${_alice.publicKey.toString('hex')}
          OP_CHECKSIG
  ...
  it(
    ...
    async () => {
      ...
      const p2sh = firovm.payments.p2sh({
        redeem: {
      ...
      const tx = new firovm.Psbt({ network: regtest })
        .setVersion(2)
        .addInput({
          hash: unspent.txId,
          index: unspent.vout,
  ...
  it(
    ...
    async () => {
      ...
      const tx = new firovm.Transaction();
      tx.version = 2;
      ...
      const redeemScriptSig = firovm.payments.p2sh({
        network: regtest,
        redeem: {
          network: regtest,
          output: p2sh.redeem!.output,
          input: firovm.script.compile([
            firovm.script.signature.encode(
              alice.sign(signatureHash),
              hashType,
            ),
            firovm.script.signature.encode(bob.sign(signatureHash), hashType),
            firovm.opcodes.OP_TRUE,
          ]),
        },
      }).input;
...
function csvGetFinalScripts(
  ...
  const decompiled = firovm.script.decompile(script);
  ...
  if (!decompiled || decompiled[0] !== firovm.opcodes.OP_IF) {
    throw new Error(`Can not finalize input #${inputIndex}`);
  }

  // Step 2: Create final scripts
  let payment: firovm.Payment = {
    network: regtest,
    output: script,
  ...
  if (isP2WSH && isSegwit)
    payment = firovm.payments.p2wsh({
      network: regtest,
      redeem: payment,
    });
  if (isP2SH)
    payment = firovm.payments.p2sh({
      network: regtest,
      redeem: payment,
    });
