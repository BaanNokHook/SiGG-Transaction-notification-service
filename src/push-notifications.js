...
const SERVICE_WORKER_URL = `/service-worker.js?pusherBeamWebSDKversion=${sdkVersion}`;  
...
export class Client {
      constructor(config) { 
            ...
            if (!('indexdDB' in window)) {  
                  throw new Error(
                        'Pusher Beams does not support this browser version (IndexDB not supported)'   
                  );
            }
