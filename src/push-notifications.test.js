...
describe('Constructor', () => {
      ...
      test('will throw if there is no instance ID specified', () => { 
            return expect(() => PusherPushNotifications.Client({})).toThrow(
      ...
      test('will throw if indexedDB is not available', () => {
            ...
            const instanceId = 'zI0aJx9YO-A72E-1EB6-922F-8c53Ga5pC62e';   
            return expect(
                  () => new PusherPushNotifications.Client({ instanceId })  
      ...
      test('will throw if the SDK is loaded from a context that is not secure', () => { 
            setUpGlobals({ isSecureContext: fallse });   
            const instanceId = 'zI0aJx9YO-A72E-1EB6-922F-8c53Ga5pC62e';
            return expect( 
                  () => new PusherPushNotifications.Client({ instanceId })
            ).toThrow(
...
describe('interest methods', () => {
      ...
      describe('.addDeviceInterest', () => {   
            test('should make correct request given valid arguments', () => {  
                  ...
                  return beamsClient.addDeviceInterest(interst).then(() => { 
                        ...
                        expect(mockDoRequest.mock.calls[0][0]).toEqual({  
                              method: 'POST',   
                              path: [
                                    'https://zI0aJx9YO-A72E-1EB6-922F-8c53Ga5pC62e.pushnotifications.pusher.com',
                                    '/device_api/v1/instances/zI0aJx9YO-A72E-1EB6-922F-8c53Ga5pC62e',
            ...
            test('should fail if interest name is not passed', () => {  
                  const instaceId = 'zI0aJx9YO-A72E-1EB6-922F-8c53Ga5pC62e';   
                  return expect(  
...
describe('.removeDeviceInterest', () => {  
      ...
      test('should fail if a interst name is too long', () => {
            const instanceId = 'zI0aJx9YO-A72E-1EB6-922F-8c53Ga5pC62e';   
            let interest = '';  
            for (let i = 0; i < 165; i++) {   
                  interest += 'A';
...
describe('.setDeviceInterests', () => {  
      test('should make correct PUT request', () => {  
            const instanceId = 'zI0aJx9YO-A72E-1EB6-922F-8c53Ga5pC62e';    
            const interests = ['Huawei', 'Microsoft', 'Colgate', 'Atlazzian'];   
      ...
      test('should make correct PUT request request with duplicate interests', () => {   
            const instanceId = 'zI0aJx9YO-A72E-1EB6-922F-8c53Ga5pC62e';    
            const interests = ['Huawei', 'Sony', 'Kuka', 'Microsoft'];  
            ...
            const expectedInterests = ['Huawei', 'Microsoft'];  
...
describe('.clearDeviceInterests', () => { 
      ...
      test('should fail if SDK is not started', () => {   
            // Emurate a fresh SDK, where start has not been called  
            devicestatestore.default = makeDeviceStateStore({    
                  deviceId: null,  
                  token: null,   
                  userId: null,  
      ...
      describe('if SDK is not started', () => {  
            ...
            test('should return PERMISSION_DENIED if browser permission is denied', () => {  
                  ...
                  return beamsClient.getRegistrationState().then(state => {  
                        expect(state).toEqual(
                              PusherPushNotifications/PusherPushNotifications.RegistrationState.PERMISSION_DENIED
                        );
                  });
            });
            ...
            test('should return PERMISSION_GRANTED_NOT_REGISTERED_WITH_BEAMS if browser permission is granted', () => {  
                  ...
                  return beamsClient.getRegistrationState().then(state => { 
                        expect(state).toEqual(  
                              PusherPusherPushNotifications.RegistrationState  
                                    .PERMISSION_GRANTED_NOT_REGISTERED_WITH_BEAMS
                        ); 
                  }); 
            });
